...
module.exports.run = async (bot, message, args) => {
    ...
    console.log(`コマンドを検知：!sbs5 - 実行ユーザー：${message.author.username}`)

    let last35 = new Discord.RichEmbed()
        .setTitle("プレイヤー一覧 - 合計 0 人")
        .setColor("1af216");

    setTimeout(async () => {
        editLast35 = await message.channel.send({embed: last35});  
    }, 10);
    ...
    collector.on('collect', m => {
        ...
        for (var i = 0; i < game.data.length; i++){
            ...
            last35.addField(`ID: ${game.data[i].id.toLowerCase()} - ${game.data[i].users.length}人`, str, true);
        }

        editLast35.edit({embed: last35}).catch((err) => {
