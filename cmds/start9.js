...
module.exports.run = async (bot, message, args) => {
    ...
    console.log(`コマンドを検知：!sbs9 - 実行ユーザー：${message.author.username}`)

    let last39 = new Discord.RichEmbed()
        .setTitle("プレイヤー一覧 - 合計 0 人")
        .setColor("1af216");

    setTimeout(async () => {
        editLast39 = await message.channel.send({embed: last39});  
    }, 10);
    ...
    collector.on('collect', m => {
        ...
        for (var i = 0; i < game.data.length; i++){
            ...
            last39.addField(`ID: ${game.data[i].id.toLowerCase()} - ${game.data[i].users.length}人`, str, true);
        }

        editLast39.edit({embed: last39}).catch((err) => {
