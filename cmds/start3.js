...
module.exports.run = async (bot, message, args) => {
    ...
    console.log(`コマンドを検知：!sbs3 - 実行ユーザー：${message.author.username}`)

    let last33 = new Discord.RichEmbed()
        .setTitle("プレイヤー一覧 - 合計 0 人")
        .setColor("1af216");

    setTimeout(async () => {
        editLast33 = await message.channel.send({embed: last33});  
    }, 10);
    ...
    collector.on('collect', m => {
        ...
        for (var i = 0; i < game.data.length; i++){
            ...
            last33.addField(`ID: ${game.data[i].id.toLowerCase()} - ${game.data[i].users.length}人`, str, true);
        }

        editLast33.edit({embed: last33}).catch((err) => {
