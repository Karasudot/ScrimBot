...
module.exports.run = async (bot, message, args) => {
    ...
    console.log(`コマンドを検知：!sbs8 - 実行ユーザー：${message.author.username}`)

    let last38 = new Discord.RichEmbed()
        .setTitle("プレイヤー一覧 - 合計 0 人")
        .setColor("1af216");

    setTimeout(async () => {
        editLast38 = await message.channel.send({embed: last38});  
    }, 10);
    ...
    collector.on('collect', m => {
        ...
        for (var i = 0; i < game.data.length; i++){
            ...
            last38.addField(`ID: ${game.data[i].id.toLowerCase()} - ${game.data[i].users.length}人`, str, true);
        }

        editLast38.edit({embed: last38}).catch((err) => {
