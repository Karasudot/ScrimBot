...
module.exports.run = async (bot, message, args) => {
    ...
    let last32 = new Discord.RichEmbed()
        .setTitle(`プレイヤー一覧 - 合計 0 人`)
        .setColor("1af216");

    setTimeout(async () => {
        editLast32 = await message.channel.send({embed: last32});  
    }, 10);

    console.log(`コマンドを検知：!sbs2 - 実行ユーザー：${message.author.username}`)
    ...
    collector.on('collect', m => {
        ...
        for (var i = 0; i < game.data.length; i++){
            ...
            last32.addField(`ID: ${game.data[i].id.toLowerCase()} - ${game.data[i].users.length}人`, str, true);
        }

        editLast32.edit({embed: last32}).catch((err) => {
