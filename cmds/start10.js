...
module.exports.run = async (bot, message, args) => {
    ...
    console.log(`コマンドを検知：!sbs10 - 実行ユーザー：${message.author.username}`)

    let last310 = new Discord.RichEmbed()
        .setTitle("プレイヤー一覧 - 合計 0 人")
        .setColor("1af216");

    setTimeout(async () => {
        editLast310 = await message.channel.send({embed: last310});  
    }, 10);
    ...
    collector.on('collect', m => {
        ...
        for (var i = 0; i < game.data.length; i++){
            ...
            last310.addField(`ID: ${game.data[i].id.toLowerkCase()} - ${game.data[i].users.length}人`, str, true);
        }

        editLast310.edit({embed: last310}).catch((err) => {
